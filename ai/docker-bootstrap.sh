...
CONSUL_DOCKER_ARGS="-d -p 8301:8301/udp -p 8302:8302/udp -p 8300:8300/tcp -p 8301:8301/tcp -p 8302:8302/tcp -p 8400:8400 -p 8500:8500 -p 53:53/udp -p 53:53/tcp -v ${CONSUL_HOST_DIR}:/data --restart always"
...
if [ -e /etc/system-release ]; then
  if [ "$(cat /etc/system-release)" == "Amazon Linux AMI release 2016.03" ]; then
    ...
    CONSUL_ARGS="${CONSUL_ARGS} -dc ${INSTANCE_REGION} -advertise ${INSTANCE_PRIV_IP} -client 0.0.0.0"
  fi
fi
